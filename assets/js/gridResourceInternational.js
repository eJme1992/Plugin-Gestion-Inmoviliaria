class GridResourceInternational {
	...
	grid() {
		const self = this;
		self.task = 'FIND_ALL_RESOURCES_INTERNATIONAL';
		self.crud = 'PROPERTY';
	...
	formatterEvent(value, row, index) {
		var ret = ['<a class="btn btn-default btn-xs view" href="javascript:void(0)" title="Ver">', '<i class="fa fa-check fa-lg"></i>', '</a>  ',
			'<a class="btn btn-danger btn-xs remove" href="javascript:void(0)" title="Eliminar">', '<i id=delete1' + row.idRecurso + ' class="fa fa-remove fa-lg"></i>', '</a>'
		];
	...
	deleteResource(e, row) {
		...
		if (confirmr === true) {
			$.ajax({
				...
				cache: false,
				beforeSend: function() {
					$('#delete1' + row.idRecurso).removeClass('class="fa fa-remove fa-lg"').prepend('<i class="fa fa-spinner fa-spin"></i>');
				},
